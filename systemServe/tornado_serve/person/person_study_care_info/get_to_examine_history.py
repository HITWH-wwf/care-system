...
class GetToExamineHistory(object):
    def entry(self,receiveRequest):
        ...
        if userName == None:
            ...
            thisStu=MyBaseModel.returnList(stu_all_aspect_info.select(stu_all_aspect_info.fushuji).where(stu_all_aspect_info.stuID==stuId))
            if len(thisStu)==0:
                return {'status':0,'errorInfo':'查询不到该学生的相关记录'}
            thisStu=thisStu[0]
            tableData=eval(thisStu['fushuji'])
            return {
                    "status": 1,
